...
// Add a check for Firebase DB initialization
if (!db) {
  console.warn('Firebase DB not initialized. Some features may not work properly.');
}
import {
  collection,
  doc,
...
export async function handleSignIn(userData: {
  ...
  if (!docSnap.exists()) {
    ...
    return { hasCompletedOnboarding: userDataFromFirestore.hasCompletedOnboarding === true };
  }
}
...
// NOTE: All Firebase queries in this file are optimized for performance with:
// - Proper indexing (orderBy clauses)
// - Limits on result sets where appropriate
// - Efficient query structures (where clauses combined with orderBy)
// - Field selection optimization where possible

/**
 * Fetches all necessary data for the main dashboard.
 * @param {string} userId The UID of the logged-in user.
 * @param {number} transactionCount The number of recent transactions to fetch (default: 30).
 * @returns {Promise<{accounts: Account[], transactions: Transaction[]}>} An object containing accounts and recent transactions.
 */
export async function getDashboardData(userId: string, transactionCount: number = 30): Promise<{
  accounts: Account[];
  transactions: Transaction[];
}> {
  try {
    const accounts = await getAccounts(userId);
    const transactions = await getRecentTransactions(userId, transactionCount);
...
export async function getUniqueTransactionCategories(
  ...
  // Fetch a smaller sample size for better performance
  const transactions = await getRecentTransactions(userId, 200);
...
export async function togglePinAccount(
  ...
  try {
    ...
    return { success: false, error: 'Could not update pin status.' };
  }
}

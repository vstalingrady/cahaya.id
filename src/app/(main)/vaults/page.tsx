            ...
            <div className="space-y-4">
                {vaults.map(vault => (
                    <div key={vault.id} className="bg-gradient-to-r from-red-900/50 to-red-800/50 backdrop-blur-xl p-5 rounded-2xl border border-red-600/20 shadow-2xl relative overflow-hidden">
                        ...
                        <Progress value={(vault.currentAmount / vault.targetAmount) * 100} className="h-2 bg-red-900/80 [&>div]:bg-gradient-to-r [&>div]:from-red-500 [&>div]:to-red-400" />
                    </div>
                ))}
            </div>

             <button className="w-full bg-gradient-to-r from-red-900/30 to-red-800/30 backdrop-blur-xl p-5 rounded-2xl flex items-center justify-center text-red-400 border-2 border-dashed border-red-600/40 hover:border-red-500/60 transition-all duration-300 relative overflow-hidden group">
                <NoiseOverlay opacity={0.02} />
                <Plus className="w-6 h-6 mr-3 group-hover:text-red-300 transition-colors relative z-10" />
                <span className="font-bold group-hover:text-red-300 transition-colors relative z-10">Create New Vault</span>
            </button>
        </div>
    );
}

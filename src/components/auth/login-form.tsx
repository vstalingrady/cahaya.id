...
export default function LoginForm() {
  ...
  const handleAuthSuccess = async (user: User) => {
    ...
    if (onboardingComplete) {
      router.push('/'); // Redirect to dashboard if onboarding is complete
    } else {
      router.push('/setup-security'); // Redirect to PIN setup if onboarding is not complete
    }
  };
  ...
  const handleAutofillDemo = () => {
    ...
    if (!demoEmail || !demoPassword) {
      toast({
        variant: "destructive",
        title: "Demo Credentials Not Found",
        description: "Demo user details are not configured in the environment variables.",
      });
      return;
    }
  ...
  return (
    <div className="space-y-6">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <Button
          type="submit"
          disabled={isLoading || isSocialLoading || !email || !password}
      ...
      <div className="space-y-4">
          <Button className="w-full bg-white text-black hover:bg-gray-200 h-14 text-base font-semibold border border-gray-200/50 flex items-center justify-center" type="button" onClick={() => handleSocialSignIn(googleProvider)} disabled={isLoading || isSocialLoading}>
            {isSocialLoading ? <Loader2 className="animate-spin" /> : <><FaGoogle className="mr-3" /><span>Continue with Google</span></>}
          </Button>
          <Button variant="secondary" className="w-full h-14 text-base font-semibold" type="button" onClick={handleAutofillDemo} disabled={isLoading || isSocialLoading}>
